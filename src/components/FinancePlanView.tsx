...
const FINANCING_COLORS: Record<string, string> = {
  presales: 'bg-[#2D7A4F]',
  gap_financing: 'bg-[#B8860B]',
  tax_credit: 'bg-violet-500',
  equity: 'bg-pink-500',
  deferral: 'bg-amber-400',
  mezzanine: 'bg-indigo-500',
  soft_money: 'bg-orange-400',
  in_kind: 'bg-teal-500',
...
export default function FinancePlanView({ plan }: FinancePlanViewProps) {
  ...
  return (
    <div className="space-y-6">
      {/* Summary */}
      <div className="bg-[#1A1714] text-[#FDFBF7] rounded-xl p-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div>
            <p className="text-[#FDFBF7]/50 text-sm">Total Budget</p>
            <p className="text-2xl font-bold">{formatCurrency(plan.totalBudget)}</p>
          </div>
          <div>
            <p className="text-[#FDFBF7]/50 text-sm">Total Secured/Projected</p>
            <p className="text-2xl font-bold">{formatCurrency(capitalStackTotal)}</p>
          </div>
          <div>
            <p className="text-[#FDFBF7]/50 text-sm">Remaining Gap</p>
            <p className={`text-2xl font-bold ${
              plan.gapAnalysis.gap <= 0 ? 'text-[#2D7A4F]' : 'text-[#C0392B]'
            }`}>
              {formatCurrency(Math.max(0, plan.gapAnalysis.gap))}
            </p>
          </div>
          <div>
            <p className="text-[#FDFBF7]/50 text-sm">Funding Level</p>
            <p className="text-2xl font-bold">
      ...
      <div className="bg-white rounded-xl border border-[#E2DACB] p-6">
        <h3 className="font-semibold text-[#1A1714] mb-4">Capital Stack</h3>

        <div className="mb-6">
          <div className="flex h-12 rounded-lg overflow-hidden border border-[#E2DACB]">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-[#F5F0E8]/50 border-b border-[#E2DACB]">
                <tr>
                  <th className="px-6 py-3 text-left text-[11px] font-medium text-[#8C8577] uppercase tracking-wider">
                    Territory
                  </th>
                  <th className="px-6 py-3 text-left text-[11px] font-medium text-[#8C8577] uppercase tracking-wider">
                    Group
                  </th>
                  <th className="px-6 py-3 text-right text-[11px] font-medium text-[#8C8577] uppercase tracking-wider">
                    MG Value
                  </th>
                  <th className="px-6 py-3 text-right text-[11px] font-medium text-[#8C8577] uppercase tracking-wider">
                    Commission
              ...
              <tbody className="divide-y divide-[#E2DACB]/60">
                {plan.salesEstimates.map((est, idx) => (
                  <tr key={idx} className="hover:bg-[#F5F0E8]/30">
                    <td className="px-6 py-3 text-[#1A1714] font-medium">
                      {est.territory}
                    </td>
                    <td className="px-6 py-3 text-[#8C8577]">
                      {est.territoryGroup || '—'}
                    </td>
                    <td className="px-6 py-3 text-right text-[#1A1714] font-medium">
                      {formatCurrency(est.mgValue)}
                    </td>
                    <td className="px-6 py-3 text-right text-[#8C8577]">
                      {est.commissionRate}%
                    </td>
                    <td className="px-6 py-3 text-right text-[#1A1714] font-medium">
              ...
              <tfoot className="bg-[#F5F0E8]/50 border-t border-[#E2DACB]">
                <tr className="font-semibold">
                  <td colSpan={2} className="px-6 py-3 text-[#1A1714]">
                    Total
                  </td>
                  <td className="px-6 py-3 text-right text-[#1A1714]">
                    {formatCurrency(
          ...
          <div className="divide-y divide-[#E2DACB]/60">
            {plan.taxIncentives.map((incentive) => (
              <div key={incentive.id} className="p-6">
                ...
                {incentive.notes && (
                  <p className="text-xs text-[#8C8577] bg-[#F5F0E8]/50 p-2 rounded-lg" style={{ fontFamily: 'Georgia, serif' }}>
                    {incentive.notes}
                  </p>
                )}
              </div>
      ...
      {plan.gapAnalysis.gap > 0 && (
        <div className="bg-[#B8860B]/5 border border-[#B8860B]/15 rounded-xl p-6">
          <h3 className="font-semibold text-[#1A1714] mb-4">
          ...
          {plan.gapAnalysis.suggestedSources.length > 0 && (
            <div>
              <p className="font-semibold text-[#1A1714] mb-3">
                Suggested Financing Sources
              </p>
              <div className="space-y-2">
                {plan.gapAnalysis.suggestedSources.map((source, idx) => (
                  <div key={idx} className="bg-white p-3 rounded-xl border border-[#E2DACB]">
